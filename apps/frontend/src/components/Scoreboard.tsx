...
import { getPartnerships } from "../utils/partnershipUtils";
...
const Scoreboard: React.FC<ScoreboardProps> = ({
  ...
  const displayInnings =
    activeTab === "current" ? currentInnings : previousInnings;
        ...
        <tbody className="space-y-2">
          {(order || []).map((id) => {
            ...
            if (
              ...
              id !== displayInnings?.strikerId &&
              id !== displayInnings?.nonStrikerId
            ) {
              return null;
            }
            ...
            const highlight =
              activeTab === "current" && (isStriker || isNonStriker);
            const sr =
              player.ballsFaced > 0
            ...
            return (
              <tr
                key={id}
                className={`group transition-all duration-300 ${highlight ? "bg-indigo-600 shadow-lg shadow-indigo-600/20" : "bg-white/5 hover:bg-white/10"}`}
              >
                <td
                  className={`px-4 py-3 first:rounded-l-xl font-black uppercase tracking-tight italic text-sm ${highlight ? "text-white" : "text-slate-200"}`}
                >
                  {player.name}{" "}
                  {isStriker && (
                    <span className="text-indigo-300 animate-pulse ml-1 text-base">
                      🏏
                    </span>
                  )}
                </td>
                ...
                <td
                  className={`px-4 py-3 text-right font-black text-base tabular-nums ${highlight ? "text-white" : "text-slate-100"}`}
                >
                  {player.runs}
                </td>
                <td
                  className={`px-4 py-3 text-right font-black text-xs tabular-nums ${highlight ? "text-indigo-200" : "text-slate-400"}`}
                >
                  {player.ballsFaced}
                </td>
                <td
                ...
                <td
                  className={`px-4 py-3 last:rounded-r-xl text-right font-black text-xs tabular-nums ${highlight ? "text-white" : "text-indigo-400"}`}
                >
                  {sr}
                </td>
              </tr>
        ...
        <tbody className="space-y-2">
          {(order || []).map((id) => {
            ...
            if (
              bowler.overs === 0 &&
              bowler.balls === 0 &&
              id !== displayInnings?.currentBowlerId
            )
              return null;
            ...
            const econ =
              totalOvers > 0
                ? (bowler.runsConceded / totalOvers).toFixed(1)
                : "-";
            const isCurrent =
              activeTab === "current" && id === displayInnings?.currentBowlerId;

            return (
              <tr
                key={id}
                className={`group transition-all duration-300 ${isCurrent ? "bg-indigo-600 shadow-lg shadow-indigo-600/20" : "bg-white/5 hover:bg-white/10"}`}
              >
                <td
                  className={`px-4 py-3 first:rounded-l-xl font-black uppercase tracking-tight italic text-sm ${isCurrent ? "text-white" : "text-slate-200"}`}
                >
                  {bowler.name}
                </td>
                <td
                  className={`px-4 py-3 text-right font-black text-xs tabular-nums ${isCurrent ? "text-indigo-200" : "text-slate-400"}`}
                >
                ...
                <td
                  className={`px-4 py-3 text-right font-black text-base tabular-nums ${isCurrent ? "text-white" : "text-slate-100"}`}
                >
                  {bowler.runsConceded}
                </td>
                <td
                  className={`px-4 py-3 text-right font-black text-base tabular-nums ${isCurrent ? "text-white" : "text-indigo-400"}`}
                >
                  {bowler.wickets}
                </td>
                <td
                  className={`px-4 py-3 last:rounded-r-xl text-right font-black text-xs tabular-nums opacity-60 ${isCurrent ? "text-white" : "text-slate-400"}`}
                >
                  {econ}
                </td>
              </tr>
            ...
            return (
              <div className="bg-indigo-600 py-8 text-center overflow-hidden relative shadow-2xl">
                ...
                <div className="relative z-10 flex flex-col items-center justify-center px-4">
                  <span className="text-[10px] font-black text-indigo-100 uppercase tracking-[0.4em] opacity-80 mb-2 leading-none">
                    🏆 MATCH RESULT
                  </span>
        ...
        {displayInnings ? (
          <div className="p-8 overflow-y-auto pb-20 scrollbar-hide space-y-12">
            ...
            <div>
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="text-right">
                  ...
                  <span className="ml-3 text-xs font-black text-slate-500 bg-white/5 px-2 py-1 rounded uppercase tracking-tighter tabular-nums flex items-baseline gap-1">
                    <span>
                      {displayInnings.overs}.{displayInnings.balls}
                    </span>
                    {totalOvers && (
                      <>
              ...
              <BattingTable
                players={displayInnings.players}
                order={displayInnings.battingOrder}
              />
            </div>
            ...
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 mb-6">
                <div className="flex flex-col">
                  <span className="text-[7px] text-slate-500 mb-0.5 tracking-[0.2em]">
                    {displayInnings.bowlingTeamName}
                  </span>
                  <span className="text-[10px] font-black uppercase tracking-widest text-indigo-400 italic">
                    Bowling Strategy
                  </span>
              ...
              <BowlingTable
                bowlers={displayInnings.bowlers}
                order={displayInnings.bowlingOrder}
              />
            </div>
                ...
                <table className="w-full text-left border-separate border-spacing-y-2">
                  <thead>
                    <tr className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-500">
                      <th className="px-2 pb-2 w-16">Wkt</th>
                      <th className="px-2 pb-2">Runs (Balls)</th>
                      <th className="px-2 pb-2">Dismissed</th>
                    </tr>
                  </thead>
                  <tbody>
                    {getPartnerships(displayInnings.allBalls).length === 0 && (
                      <tr>
                        <td
                          colSpan={3}
                          className="text-[10px] text-slate-500 text-center py-2 uppercase"
                        >
                          No partnerships yet
                        </td>
                      </tr>
                    )}
                    {getPartnerships(displayInnings.allBalls).map((p, idx) => (
                      <tr key={idx} className="bg-white/5">
                        <td className="px-2 py-2 rounded-l-xl font-black text-xs text-slate-400">
                          {p.wicketNumber}
                        </td>
                        <td className="px-2 py-2 font-black text-indigo-300">
                          {p.runs}{" "}
                          <span className="text-slate-500 text-[10px] ml-1">
                            ({p.balls})
                          </span>
                        </td>
                        <td className="px-2 py-2 rounded-r-xl font-bold text-[10px] text-slate-300 uppercase tracking-widest">
                          {p.outBatterName || "Unbroken"}
                        </td>
                      </tr>
                    ))}
                  </tbody>
                    ...
                    {(() => {
                      ...
                      displayInnings.allBalls.forEach((ball) => {
                        if (ball.extraType === ExtraType.WIDE)
                          w += ball.extraRuns + ball.runs;
                        if (ball.extraType === ExtraType.NO_BALL)
                          nb += ball.extraRuns + ball.runs;
                        if (ball.extraType === ExtraType.BYE)
                          b += ball.extraRuns + ball.runs;
                        if (ball.extraType === ExtraType.LEG_BYE)
                          lb += ball.extraRuns + ball.runs;
                      });
                      return (
                        <>
                          <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest block">
                            Extras Total: {w + nb + b + lb}
                          </span>
          ...
          <div className="p-32 text-center text-slate-600 font-black uppercase italic tracking-[0.5em] opacity-20">
            Secure Data Lost
          </div>
        )}
      </div>
