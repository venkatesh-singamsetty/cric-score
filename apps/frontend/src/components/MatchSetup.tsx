...
const MatchSetup: React.FC<MatchSetupProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAlertMessage(
        "Cloud Connection Failed. Check your Aiven database status.",
      );
    } finally {
      setIsCreating(false);
    }
                ...
                {recentMatches.map((m) => (
                  ...
                  >
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${m.status === "LIVE" ? "bg-rose-500 animate-pulse" : "bg-slate-600"}`}
                      ></span>
                    </div>
          ...
          >
            ...
            <div className="relative flex flex-col items-center justify-center h-full">
              <div className="flex items-center gap-4">
                <span
                  className={`text-xl font-black uppercase italic transition-all ${isValid && !isCreating ? "text-white tracking-[0.4em] group-hover:tracking-[0.5em]" : "text-slate-500 tracking-[0.2em]"}`}
                >
